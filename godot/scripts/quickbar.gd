...
func _process(delta: float) -> void:
	if not game: return
	bar.visible = game.started and not game.over and game.player.alive and (game.player.active or game.inventory.is_open)
	if not bar.visible:
		picker.hide()
		return
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if not game.player.active or get_tree().paused: return
