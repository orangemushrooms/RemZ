...
func _update_animation(delta: float) -> void:
	...
	if _charge_t > 0.0: pitch -= 0.22   # head down for the ram
	model.position.y = _base_y + bob
	model.position.z = lunge * float(type.beast.get("lunge", 0.4))
	model.rotation.x = pitch - lunge * 0.3
...
func die(dir: Vector3) -> void:
	...
	tween.tween_property(model, "position:y", _base_y - height * 0.28, 0.45).set_ease(Tween.EASE_IN)
