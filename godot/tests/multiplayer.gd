...
func host_run() -> void:
	...
	await wait_seconds(0.4)
	check(not game.over and NetSession.world.actor(c2).downed and NetSession.world.actor(c2).alive, "One player down does not end coop")
	check(game.stats.downs == 1, "The down is recorded")
	await command_clients("revive", ["c3"], [c2])
	await wait_seconds(3.6)
	check(NetSession.world.actor(c2).alive and not NetSession.world.actor(c2).downed and NetSession.world.actor(c2).hp >= 50.0, "Host completes the three-second revive")
	await command_clients("inspect", ["c2"])
	...
	check(not NetSession.world.actor(c2).alive and not game.over, "Bleeding out kills the player; the team fights on")
	await command_clients("revive", ["c3"], [c2])
	await wait_seconds(3.6)
	check(NetSession.world.actor(c2).alive and NetSession.world.actor(c2).hp >= 50.0, "A dead teammate is revived by hand")
