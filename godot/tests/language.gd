...
func _sweep_menus() -> void:
	...
	await _frames(2)
	# tower build menu (T): ground placement, then a roof slot of the forest hut
	game.defences.begin_building()
	await _frames(2)
	await _sweep("tower build menu", game.defences.build_menu)
	game.defences.site_picker.select(1)
	game.defences.site_picker.item_selected.emit(1)
	game.defences._refresh_build_menu()
	await _frames(2)
	await _sweep("tower build menu / roof slot", game.defences.build_menu)
	game.defences.close()
	await _frames(2)
	# drone control console; away from the station its rows explain why no drone can start
	game.drones._refresh_menu()
	game.drones.panel.show()
	await _frames(2)
	await _sweep("drone console", game.drones.ui)
	game.drones.panel.hide()
	await _frames(2)
