...
func run() -> void:
	...
	await shot("waldlaeufer-hip")
	Input.action_press("aim")
	w._handle_weapon_input(1.0)
	var aimed_width := p.camera.unproject_position(edge).distance_to(p.camera.unproject_position(centre))
	check(is_equal_approx(aimed_width / hip_width, 4.0), "Waldlaeufer magnifies actual projected world geometry by 4x")
