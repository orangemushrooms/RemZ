...
func run() -> void:
	...
	bar.refresh()
	check(Lang.text(bar.ammo_badges[2].text) == "FIRE 2" and bar.counts[2].text.contains("/"), "Selected gun shows fire rounds alongside normal ammunition")
	game.weapons.set_weapon("knife")
	bar.refresh()
	check(bar.ammo_badges[2].text.is_empty() and bar.ammo_badges[9].text.is_empty(), "Melee selection clears the gun badge without labelling the knife")
	game.weapons.set_weapon("pistol")
	game.progression.rare_market.consume_round(game.player)
	bar.refresh()
	check(Lang.text(bar.ammo_badges[2].text) == "FIRE 1", "Fire badge follows remaining special rounds")
	game.progression.rare_market.consume_round(game.player)
	bar.refresh()
	check(bar.ammo_badges[2].text.is_empty(), "Empty special ammunition clears the badge")
	rare.ammo.frost = 12
	game.progression.rare_market.equip(game.player, "frost")
	bar.refresh()
	check(Lang.text(bar.ammo_badges[2].text) == "FROST 12", "Frost selection uses the same weapon badge")
	game.progression.rare_market.equip(game.player, "normal")
	bar.refresh()
	check(bar.ammo_badges[2].text.is_empty(), "Switching to normal ammunition clears the badge")
