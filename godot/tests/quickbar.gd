...
func run() -> void:
	...
	bar.refresh()
	check(bar.ammo_badges[2].text == "FEUER 2" and bar.counts[2].text.contains("/"), "Selected gun shows fire rounds alongside normal ammunition")
	...
	bar.refresh()
	check(bar.ammo_badges[2].text == "FEUER 1", "Fire badge follows remaining special rounds")
	game.progression.rare_market.consume_round(game.player)
	bar.refresh()
	...
	bar.refresh()
	check(bar.ammo_badges[2].text == "FROST 12", "Frost selection uses the same weapon badge")
