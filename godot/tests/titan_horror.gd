...
func run() -> void:
	...
	for clip in TitanPresence.SPAWN_CLIPS:
		var stream: AudioStream = load(Sfx.DIR + clip)
		check(stream is AudioStreamMP3 and stream.get_length() > 3 and not stream.loop, "User spawn recording loads without looping: " + clip)
	...
	check(presence._voices[0].max_distance == 0 and presence._voices[0].attenuation_model == AudioStreamPlayer3D.ATTENUATION_DISABLED and presence._voices[0].global_position.y > titan.global_position.y + 15, "Spawn cry has unlimited reach while retaining the giant's direction")
	check(presence._voices[0].stream.resource_path == Sfx.DIR + TitanPresence.SPAWN_CLIPS[posmod(titan.appearance_seed + 1, 4)], "Spawn uses the recording chosen by the host's random titan seed")
